...
const Utility = preload("Utility.gd")
...
# input buffer addition
# the buffer is NOT thread safe!
@export var input_buffer: Array[MidiInputNoteResource] = []

var last_filter_frame: int = 0
...
func filter_old_input_buffer(time_offset_in_ms: int = 1000) -> void:
	...
	last_filter_frame = current_frame
...
func _process_track_event_note_on(channel: GodotMIDIPlayerChannelStatus, note: int, velocity: int) -> void:
	...
	if note_on:
		...
		#" , modulation_sensitivity: ", channel.rpn.modulation_sensitivity,
		" , auto_release_mode: ", channel.drum_track,
		#" , polyphony_count: ", polyphony_count,
		" , hold: ", channel.hold,
		)
